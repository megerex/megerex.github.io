...
```{r, echo=FALSE, eval=TRUE, message=FALSE, tidy=TRUE, warning=FALSE, error=FALSE}
    require(ggplot2)
    p <- ggplot(data.frame(x=c(-5,5)), aes(x=x, color="red")) +
             stat_function(fun=dnorm, geom = "line", size=1, n=1000) +
             annotate("text", x=-5+0.2, y=0.2, parse=TRUE, label="y==frac(1,sqrt(2*pi))*e^(frac(-x^2,2))", color="blue")
    p
```
